...
type
  ...
  protected //Operations for parameters or Binary Operators
    function stoOperation(parA, parB: TMirOperand): TStoOperandsBSIF; inline;
    procedure Exchange(var parA, parB: TMirOperand);
    function BinOperationStr(fun: TMirOperand): string;
  private
  ...
  protected  //Register work
    ...
    procedure LoadToWR(const fun: TMirOperand);
...
//    procedure GenCodeBody(body: TEleBody);
  ...
  protected  //Memory managing routines for variables
    procedure WriteVaLueToRAM(target: PtrTCPURam; add: word; typ: TEleTypeDec;
      const value: TMirConsValue);
    procedure CreateVarInRAM(xVar: TMirVarDec; shared: boolean);
    procedure CreateValueInCode(typ: TEleTypeDec; const value: TMirConsValue; out
      startAddr: integer);
  protected  //Methods for set a function result.
    procedure SetFunNull(var fun: TMirOperand);
    procedure SetFunConst(var fun: TMirOperand);
    procedure SetFunVariab(var fun: TMirOperand; vardec: TMirVarDec);
    procedure SetFunVariab(var fun: TMirOperand; vardec: TEleVarDec);
    procedure SetFunVariab(var fun: TMirOperand; addr: word);
    procedure SetFunVariab_RamVarOf(var fun: TMirOperand; vardec: TEleVarDec;
      offset: integer; offsetVar: TEleVarDec);
    procedure SetFunExpres(fun: TMirOperand);
    //Set result as a constant.
    procedure SetFunConst_bool(fun: TMirOperand; valBool: Boolean);
    procedure SetFunConst_byte(fun: TMirOperand; valByte: integer);
    procedure SetFunConst_char(fun: TMirOperand; valByte: integer);
    procedure SetFunConst_word(fun: TMirOperand; valWord: integer);
    procedure SetFunConst_dword(fun: TMirOperand; valWord: DWord);
...
{ TGenCodPic }
function TGenCodBas.BinOperationStr(fun: TMirOperand): string;
...
function TGenCodBas.stoOperation(parA, parB: TMirOperand): TStoOperandsBSIF;
begin
  //Combinación de los almacenamientos de los operandos
  Result := TStoOperandsBSIF((Ord(parA.Sto) << 4) or ord(parB.Sto));
end;
procedure TGenCodBas.Exchange(var parA, parB: TMirOperand);
...
procedure TGenCodBas.LoadToWR(const fun: TMirOperand);
...
procedure TGenCodBas.CreateVarInRAM(xVar: TMirVarDec; shared: boolean);
...
begin
  ...
  if outOfProgram then begin
    ...
    if startAdd+nbytes-1>high(pic.ram) then begin
      GenError('Cannot allocate variable: %s', [varName], xVar.vardec.srcDec);
      exit;
    end;
  ...
  if xVar.adicPar.hasInit then begin
    if outOfProgram then begin  //Only allowed in the program block
      GenError('Cannot initialize variable "%s" in location $%x.',
                       [varName, startAdd], xvar.vardec.srcDec);
    end;
...
procedure TGenCodBas.CreateValueInCode(typ: TEleTypeDec;
  const value: TMirConsValue; out startAddr: integer);
...
procedure TGenCodBas.SetFunNull(var fun: TMirOperand);
...
procedure TGenCodBas.SetFunConst(var fun: TMirOperand);
...
procedure TGenCodBas.SetFunVariab(var fun: TMirOperand; vardec: TMirVarDec);
...
procedure TGenCodBas.SetFunVariab(var fun: TMirOperand; vardec: TEleVarDec);
{Set an operand TxpEleExpress to type otVariab and storage stRamFix.
...
procedure TGenCodBas.SetFunVariab(var fun: TMirOperand; addr: word);
{Fija los parámetros del resultado de una subexpresion. Este método se debe ejecutar,
siempre antes de evaluar cada subexpresión.}
begin
...
procedure TGenCodBas.SetFunVariab_RamVarOf(var fun: TMirOperand; vardec: TEleVarDec;
  offset: integer; offsetVar: TEleVarDec);
...
procedure TGenCodBas.SetFunExpres(fun: TMirOperand);
...
procedure TGenCodBas.SetFunConst_bool(fun: TMirOperand; valBool: Boolean);
begin
  fun.SetCon_Literal(valBool);
end;
procedure TGenCodBas.SetFunConst_byte(fun: TMirOperand; valByte: integer);
begin
  if (valByte <0) or (valByte>=256) then begin
    GenError('Numeric value exceeds a byte range.', fun.astOperand.srcDec);
    exit;
  end;
//  SetFunConst(fun);
//  fun.evaluated := true;
...
procedure TGenCodBas.SetFunConst_char(fun: TMirOperand; valByte: integer);
begin
//  SetFunConst(fun);
//  fun.evaluated := true;
//  fun.value.valInt := valByte;
...
procedure TGenCodBas.SetFunConst_word(fun: TMirOperand; valWord: integer);
begin
  if (valWord <0) or (valWord>=65536) then begin
    GenError('Numeric value exceeds a word range.', fun.astOperand.srcDec);
    exit;
  end;
//  SetFunConst(fun);
//  fun.evaluated := true;
...
procedure TGenCodBas.SetFunConst_dword(fun: TMirOperand; valWord: DWord);
begin
  if (valWord <0) or (valWord>$FFFFFFFF) then begin
    GenError('Numeric value exceeds a dword range.', fun.astOperand.srcDec);
    exit;
  end;
//  SetFunConst(fun);
//  fun.evaluated := true;
...
//////////////// Tipo Byte /////////////
procedure TGenCodBas.byte_LoadToWR(fun: TMirOperand);
...
procedure TGenCodBas.word_LoadToWR(fun: TMirOperand);
...
procedure TGenCodBas.word_Low(var fun: TMirOperand);
...
procedure TGenCodBas.word_High(var fun: TMirOperand);
...
procedure TGenCodBas.GenCodeSentences2(sentList: TMirElements);
{Generate code for a list of sentences.}
var
