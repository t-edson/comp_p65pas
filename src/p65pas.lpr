...
type
  ...
  private
    ...
    procedure CompilerError(errTxt: string; const msgInfo: TMsgInfo);
    procedure CompilerInfo(infTxt: string; const msgInfo: TMsgInfo);
    ...
    procedure CompilerWarning(warTxt: string; const msgInfo: TMsgInfo);
    procedure PrintHelp;
  public
    procedure Compile;
    constructor Create;
...
var
  ...
  { TProg }
procedure TProg.CompilerError(errTxt: string; const msgInfo: TMsgInfo);
begin
  inc(nErrors);
  writeln('ERROR: ' + errTxt);
end;
procedure TProg.CompilerWarning(warTxt: string; const msgInfo: TMsgInfo);
begin
  writeln('WARN: ' + warTxt);
end;
procedure TProg.CompilerInfo(infTxt: string; const msgInfo: TMsgInfo);
begin
  writeln('INFO: ' + infTxt);
end;
...
begin
  ...
  writeln('By Tito Hinostroza - 2026. All rights reserved.');
  writeln('');
...
begin
  ...
  writeln('  -Fo<path> Set the path of the output *.hex file.');
  writeln('  -Dn    Disable output generated by directives');
end;
procedure TProg.CompilerMsg(msgKind: TMessageKind; const msg: string;
  const msgInfo: TMsgInfo);
{Procesa los mensajaes que genera el compilador}
begin
  case msgKind of
    mkInfo:
      CompilerInfo(msg, msgInfo);
    mkWarning:
      CompilerWarning(msg, msgInfo);
    mkError:begin
      CompilerError(msg, msgInfo);
    end;
  else ;
  end;
end;
