...
type
  ...
  private
    ...
    procedure CompilerError(const msgInfo: TMsgInfo);
    procedure CompilerInfo(const msgInfo: TMsgInfo);
    procedure CompilerMessageBox(txt: string; mode: integer);
    procedure CompilerMsg(msgKind: TMessageKind; const msgInfo: TMsgInfo);
    procedure CompilerWarning(const msgInfo: TMsgInfo);
    procedure PrintHelp;
  public
    procedure Compile;
    constructor Create;
...
var
  ...
  { TProg }
procedure TProg.CompilerError(const msgInfo: TMsgInfo);
begin
  inc(nErrors);
  writeln('ERROR: ' + msgInfo.txt);
end;
procedure TProg.CompilerWarning(const msgInfo: TMsgInfo);
begin
  writeln('WARN: ' + msgInfo.txt);
end;
procedure TProg.CompilerInfo(const msgInfo: TMsgInfo);
begin
  writeln('INFO: ' + msgInfo.txt);
end;
...
procedure TProg.CompilerMsg(msgKind: TMessageKind; const msgInfo: TMsgInfo);
{Procesa los mensajaes que genera el compilador}
begin
  case msgKind of
    mkInfo:
      CompilerInfo(msgInfo);
    mkWarning:
      CompilerWarning(msgInfo);
    mkError:begin
      CompilerError(msgInfo);
    end;
  else ;
  end;
end;
