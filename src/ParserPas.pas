...
private   //Métodos auxiliares para las declaraciones
  ...
  function ParseSubrangeType: TSubranTypeDef;
...
function TParserPas.curFile: string;
{Devuelve el archivo que se está procesando actualmente.}
begin
  Exit(lex.curCtx.fileSrc);
end;
...
begin
  ...
  Result := TSubranTypeDef.Create(LowExpr, HighExpr, lex.GetSrcPos);
end;
...
begin
  ...
  ProcType := TProcedTypeDef.Create(IsFunction, lex.GetSrcPos);
...
begin
  ...
  while not HayError do begin
    ...
    lanzar el evento callUnitAdded() con nombres duplicados}
    for untNod in unitContainer do begin
      if CompareText(untNod.UnitName, untName) = 0 then begin
        GenError(ER_DUPLIC_UNIT);   //Ya está en la lista.
        Exit;
      end;
    end;
    ...
    callUnitAdded(untName);   //Genera el evento para el gestor de unidades.
...
A diferencia de ParseFile(), no se limpian los contextos previos, sino que se agrega uno
nuevo y se cierra al terminar la exploración.}
begin
  //lex.ClearContexts;   //No limpiamos. Agregamos un nuevo contexto.
  //Lee el archivo indicado
  if not lex.OpenContextFrom(mainFile) then begin
    //No lo encuentra
    GenError(Format(ER_FIL_NOFOUND, [mainFile]));
  ...
  //Estamos en otro contexo. Analizamos el archivo.
  SkipWhites;   //OJO: Se procesan directivas aquí
